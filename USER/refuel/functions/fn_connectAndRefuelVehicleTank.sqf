 ...
 * modified to enable refueling cargotank -- GRAD
 *
 * Arguments:
 * 0: Unit <OBJECT>
 * 1: Vehicle <OBJECT>
...
#define QGVAR(var) QUOTE(TRIPLES(ace,refuel,var))
#define FUNC(var) TRIPLES(DOUBLES(ace,refuel),fnc,var)
#define EFUNC(var1,var2) TRIPLES(DOUBLES(ace,var1),fnc,var2)
#define QEGVAR(var1,var2) QUOTE(TRIPLES(ace,var1,var2))
#define TIME_PROGRESSBAR(X) (X)
#define REFUEL_PROGRESS_DURATION 1
...
if (_bestPosASL isEqualTo []) exitWith {
    ["cannot attach"] call EFUNC(common,displayTextStructured);
};
